...
class isearch_end(commands.Command):
    def do(self):
        ...
        r.pop_input_trans()
        r.dirty = 1
        r.isearch_previous_term = r.isearch_term
...
class HistoricalReader(R):
    ...
    def get_item(self, i):
        assert i>=0 and i<=len(self.history)
    ...
    def isearch_next(self):
        ...
        # If the isearch key is pressed twice without entering anything,
        # try to use the previous search term, just like Emacs does it.
        if st == '': 
            self.isearch_term=st=self.isearch_previous_term
            if st=='':
                self.error("nothing to search")
                return
        p = self.pos
        i = self.historyi
        s = self.get_unicode()
